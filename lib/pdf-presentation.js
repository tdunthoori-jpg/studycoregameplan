...
function slidePhase(d, ph, idx, n) {
  ...
  // Accept any week that exists — try sessions first, fall back to homework bullets
  const weeks   = (ph.weeks || []).filter(w => w != null);
  const wkCount = weeks.length;
  const gridCols = wkCount <= 4 ? (wkCount || 1) : wkCount <= 6 ? 3 : 4;
  const cardFont = wkCount >= 5 ? 13 : 14;
  const cardHdr  = 11;

  const weekCards = weeks.map((wk, wi) => {
    const wkNum      = wk.weekNum || wk.week || wk.weekNumber || String(wi + (ph.wkStart || 1));
    // sessionContent is the correct field name from the game plan schema
    const sessionRaw = (wk.sessionContent || wk.sessions?.[0] || wk.focus || wk.focus_area || '').trim();
    const hwItems    = (wk.homework || []).filter(h => h && h.trim());
    // Truncate long session descriptions to keep cards readable
    const line1 = sessionRaw.length > 110 ? sessionRaw.slice(0, 107) + '…' : sessionRaw || hwItems[0] || '';
    const line2 = sessionRaw && hwItems[0] ? hwItems[0].slice(0, 90) : '';
    return `<div style="background:white;border-radius:12px;padding:18px 22px;border-top:4px solid ${color};box-shadow:0 2px 12px rgba(0,0,0,0.07);display:flex;flex-direction:column;overflow:hidden;">
      <div style="font-size:${cardHdr}px;font-weight:700;color:${color};letter-spacing:0.14em;text-transform:uppercase;margin-bottom:10px;font-family:'Inter',sans-serif;">WEEK ${wkNum}</div>
      ${line1 ? `<div style="font-size:${cardFont}px;font-weight:600;color:#1E293B;line-height:1.5;font-family:'Inter',sans-serif;">${esc(line1)}</div>` : ''}
      ${line2 ? `<div style="font-size:${cardFont - 1}px;font-weight:500;color:#475569;line-height:1.4;margin-top:7px;font-family:'Inter',sans-serif;">${esc(line2)}</div>` : ''}
    </div>`;
  }).join('');

  const hasWeeks = wkCount > 0;

  // Phase goal: phaseComplete stripped of boilerplate prefix, first 2 sentences
  ...
  // Description font — scales with length so it fills vertical space
  const descLen  = (ph.description || '').length;
  const descFont = hasWeeks
    ? (descLen > 400 ? 18 : descLen > 250 ? 20 : 22)
    : (descLen > 400 ? 21 : descLen > 250 ? 24 : 26);
    ...
    <div style="flex:1;display:flex;overflow:hidden;min-height:0;">
      ...
      <div style="width:520px;flex-shrink:0;background:${color};padding:52px 48px;display:flex;flex-direction:column;position:relative;overflow:hidden;">
        ...
        <div style="position:relative;flex:1;display:flex;flex-direction:column;">
          ...
          <!-- Flex spacer pushes progress to bottom -->
          <div style="flex:1;"></div>
          <!-- Phase progress indicator -->
          <div style="display:flex;gap:8px;align-items:center;">
            ${Array.from({length: total}, (_, i) => `<div style="width:${i === idx ? '28px' : '10px'};height:4px;border-radius:2px;background:${i === idx ? 'white' : 'rgba(255,255,255,0.25)'};transition:width 0.2s;"></div>`).join('')}
          </div>
      ...
      <!-- Right: description + phase goal / weekly breakdown -->
      <div style="flex:1;padding:52px 72px 52px;background:#F8FAFC;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
        <!-- Description -->
        <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:20px;font-family:'Inter',sans-serif;flex-shrink:0;">WHAT WE'RE DOING</div>
        <div style="font-size:${descFont}px;font-weight:600;color:#1E293B;line-height:1.72;font-family:'Inter',sans-serif;flex-shrink:0;">${esc(ph.description || ph.shortDesc || '')}</div>
        <div style="height:1px;background:#E2E8F0;margin:36px 0 32px;flex-shrink:0;"></div>
        ${hasWeeks ? `
          <!-- Week-by-week grid -->
          <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:20px;flex-shrink:0;font-family:'Inter',sans-serif;">WEEK BY WEEK</div>
          <div style="display:grid;grid-template-columns:repeat(${gridCols},1fr);gap:16px;flex:1;min-height:0;align-content:start;">${weekCards}</div>
        ` : phaseGoal ? `
          <!-- Phase goal fills remaining space when no weekly data -->
          <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:20px;flex-shrink:0;font-family:'Inter',sans-serif;">PHASE GOAL</div>
