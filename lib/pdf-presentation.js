...
function slidePhase(d, ph, idx, n) {
  ...
  // Dot center from top of node: week-label line-height(18px) + margin-top(10px) + half-dot(7px) = 35px
  // Test ring (22px, margin-top 5px): 18 + 5 + 11 = 34px ≈ same
  const DOT_CENTER = 35;

  const nodeHtml = groups.map(grp => {
    const label = nodeLabel(grp);
    const wk0   = grp.weeks[0];

    if (grp.isTest) {
      const txt = esc((wk0.testTarget || 'Practice test').replace(/\s*\|.*$/, '').trim());
      return `<div style="flex:1;display:flex;flex-direction:column;align-items:flex-start;padding-right:20px;position:relative;">
        <div style="font-size:13px;font-weight:700;color:rgba(255,255,255,0.4);letter-spacing:0.08em;text-transform:uppercase;font-family:'Inter',sans-serif;line-height:18px;">${label}</div>
        <div style="width:22px;height:22px;border-radius:50%;border:2px solid #1A8870;display:flex;align-items:center;justify-content:center;margin:5px 0 12px;position:relative;z-index:1;">
          <div style="width:10px;height:10px;border-radius:50%;background:#1A8870;"></div>
        </div>
        <div style="font-size:15px;font-weight:600;color:#5ecaa5;line-height:1.55;font-family:'Inter',sans-serif;">${txt}</div>
      </div>`;
    }

    const sessionRaw = (wk0.sessionContent || wk0.sessions?.[0] || wk0.focus || '').trim();
    const hwItems    = (wk0.homework || []).filter(h => h && h.trim());
    const topic      = extractTopic(sessionRaw) || extractTopic(hwItems[0] || '');
    const more       = grp.weeks.length > 1
      ? `<div style="font-size:13px;color:rgba(255,255,255,0.35);margin-top:4px;font-family:'Inter',sans-serif;">+ ${grp.weeks.length - 1} more</div>`
      : '';
    return `<div style="flex:1;display:flex;flex-direction:column;align-items:flex-start;padding-right:20px;position:relative;">
      <div style="font-size:13px;font-weight:700;color:rgba(255,255,255,0.4);letter-spacing:0.08em;text-transform:uppercase;font-family:'Inter',sans-serif;line-height:18px;">${label}</div>
      <div style="width:14px;height:14px;border-radius:50%;background:${color};margin:10px 0 12px;position:relative;z-index:1;flex-shrink:0;"></div>
      <div style="font-size:15px;font-weight:500;color:rgba(255,255,255,0.82);line-height:1.55;font-family:'Inter',sans-serif;">${esc(topic)}${more}</div>
    </div>`;
  }).join('');

  // Description: truncate to 160 chars
  const rawDesc   = (ph.shortDesc || ph.description || '').trim();
  const descShort = rawDesc.length > 160 ? rawDesc.slice(0, 159).replace(/\s+\S*$/, '') + '…' : rawDesc;
  ...
  return `<section style="width:1920px;height:1080px;display:flex;flex-direction:column;position:relative;overflow:hidden;page-break-after:always;">
    ...
    <div style="flex:1;display:flex;overflow:hidden;min-height:0;">
      ...
      <!-- Right: dark panel — description top, timeline vertically centered in remaining space -->
      <div style="flex:1;padding:56px 80px 56px;background:#1E293B;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
        ...
        <div style="font-size:21px;font-style:italic;color:rgba(255,255,255,0.72);line-height:1.65;font-family:'Inter',sans-serif;border-left:4px solid ${color};padding-left:22px;flex-shrink:0;">${esc(descShort)}</div>
        <!-- Flex spacer — pushes timeline to vertical center of remaining space -->
        <div style="flex:1;"></div>
        <!-- Timeline section -->
        ${groups.length > 0 ? `
        <div style="flex-shrink:0;">
          <div style="font-size:12px;font-weight:700;color:rgba(255,255,255,0.28);letter-spacing:0.18em;text-transform:uppercase;font-family:'Inter',sans-serif;margin-bottom:20px;">WEEK BY WEEK</div>
          <div style="height:1px;background:rgba(255,255,255,0.15);margin-bottom:36px;"></div>
          <div style="position:relative;display:flex;align-items:flex-start;width:100%;">
            <!-- Connector line through dot centers -->
            <div style="position:absolute;top:${DOT_CENTER}px;left:0;right:0;height:1px;background:rgba(255,255,255,0.15);z-index:0;"></div>
            ${nodeHtml}
          </div>
        </div>` : ''}
        <!-- Bottom spacer — slightly smaller than top so timeline sits just below center -->
