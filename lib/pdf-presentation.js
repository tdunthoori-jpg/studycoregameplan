...
function derive(gamePlan, studentData, studentName) {
  ...
  let totalSlides = 7 + phaseCount; // 7 fixed slides + one per phase
...
function slidePhase(d, ph, idx, n) {
  ...
  const weeks    = (ph.weeks || []).filter(w => w && ((w.sessions || []).length > 0 || w.focus || w.focus_area));
  const wkCount  = weeks.length;
  const gridCols = wkCount <= 4 ? (wkCount || 1) : wkCount <= 6 ? 3 : 4;
  const cardFont = wkCount >= 5 ? 13 : wkCount >= 4 ? 14 : 15;
  const cardHdr  = wkCount >= 5 ? 11 : 12;

  const weekCards = weeks.map((wk, wi) => {
    const wkNum  = wk.week || wk.weekNumber || (wi + (ph.wkStart || 1));
    const sents  = (wk.sessions || []).filter(s => s && s.trim());
    const focus  = sents[0] || wk.focus || wk.focus_area || '';
    const focus2 = sents[1] || '';
    return `<div style="background:white;border-radius:12px;padding:16px 20px;border-top:3px solid ${color};box-shadow:0 1px 8px rgba(0,0,0,0.06);display:flex;flex-direction:column;overflow:hidden;">
      <div style="font-size:${cardHdr}px;font-weight:700;color:${color};letter-spacing:0.12em;text-transform:uppercase;margin-bottom:8px;font-family:'Inter',sans-serif;">WEEK ${wkNum}</div>
      ${focus  ? `<div style="font-size:${cardFont}px;font-weight:600;color:#1E293B;line-height:1.45;font-family:'Inter',sans-serif;">${esc(focus)}</div>` : ''}
      ${focus2 ? `<div style="font-size:${cardFont - 1}px;font-weight:500;color:#475569;line-height:1.4;margin-top:6px;font-family:'Inter',sans-serif;">${esc(focus2)}</div>` : ''}
    </div>`;
  }).join('');

  const hasWeeks = wkCount > 0;
  const descLen  = (ph.description || '').length;
  const descFont = descLen > 420 ? 19 : descLen > 260 ? 21 : 23;
  ...
  return `<section style="width:1920px;height:1080px;display:flex;flex-direction:column;position:relative;overflow:hidden;page-break-after:always;">
    ...
    <div style="flex:1;display:flex;overflow:hidden;min-height:0;">
      ...
      <div style="width:520px;flex-shrink:0;background:${color};padding:52px 48px;display:flex;flex-direction:column;position:relative;overflow:hidden;">
        <div style="position:absolute;bottom:-50px;right:-20px;font-size:300px;font-weight:800;color:rgba(255,255,255,0.07);line-height:1;font-family:'Inter',sans-serif;pointer-events:none;user-select:none;">${phNum}</div>
        <div style="font-size:11px;font-weight:700;color:rgba(255,255,255,0.5);letter-spacing:0.22em;text-transform:uppercase;font-family:'Inter',sans-serif;position:relative;">PHASE ${idx + 1}</div>
        <div style="font-size:44px;font-weight:800;color:white;line-height:1.2;margin-top:14px;font-family:'Inter',sans-serif;position:relative;">${esc(ph.shortName || ph.title)}</div>
        <div style="height:1px;background:rgba(255,255,255,0.2);margin:36px 0;flex-shrink:0;position:relative;"></div>
        <div style="display:flex;flex-direction:column;gap:24px;position:relative;">
          <div>
            <div style="font-size:80px;font-weight:800;color:white;line-height:1;font-family:'Inter',sans-serif;">${ph.hours || '—'}</div>
            <div style="font-size:14px;font-weight:600;color:rgba(255,255,255,0.55);margin-top:4px;font-family:'Inter',sans-serif;">hours</div>
          </div>
          ${wkLabel ? `<div style="font-size:18px;font-weight:700;color:rgba(255,255,255,0.8);font-family:'Inter',sans-serif;">${esc(wkLabel)}</div>` : ''}
          ${ph.season ? `<div style="display:inline-block;background:rgba(255,255,255,0.15);border-radius:8px;padding:6px 14px;font-size:13px;font-weight:700;color:rgba(255,255,255,0.85);letter-spacing:0.1em;text-transform:uppercase;font-family:'Inter',sans-serif;align-self:flex-start;">${esc(ph.season)}</div>` : ''}
        </div>
      </div>
      <!-- Right: description + optional weekly breakdown -->
      <div style="flex:1;padding:52px 72px 48px;background:#F8FAFC;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:18px;font-family:'Inter',sans-serif;flex-shrink:0;">WHAT WE'RE DOING</div>
        <div style="font-size:${descFont}px;font-weight:600;color:#1E293B;line-height:1.7;font-family:'Inter',sans-serif;flex-shrink:0;">${esc(ph.description || ph.shortDesc || '')}</div>
        ${hasWeeks ? `
        <div style="height:1px;background:#E2E8F0;margin:32px 0 28px;flex-shrink:0;"></div>
        <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:18px;flex-shrink:0;font-family:'Inter',sans-serif;">WEEK BY WEEK</div>
        <div style="display:grid;grid-template-columns:repeat(${gridCols},1fr);gap:14px;flex:1;min-height:0;align-content:start;">${weekCards}</div>
        ` : ''}
      </div>
    </div>
    ${monogramWatermark()}${slideFooter(n, d.totalSlides, d.testLabel)}
