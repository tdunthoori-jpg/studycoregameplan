...
function derive(gamePlan, studentData, studentName) {
  ...
  if (isACT) {
    ...
    // Use the last practiceTests entry for section targets — scoreProgression.rows
    // is no longer generated by Claude, so we read from the canonical array.
    const _lastPT     = (gamePlan.practiceTests || []).slice(-1)[0];
    const _rwTarget   = parseTargetNum(_lastPT?.targetRW)   || Math.round(target * 0.47);
    const _mathTarget = parseTargetNum(_lastPT?.targetMath) || Math.round(target * 0.53);
