...
function derive(gamePlan, studentData, studentName) {
  ...
  // Merge: add synthetic rows for weeks in programOverview but not scoreProgression
  const mergedWkMap = new Map(scoreRowByWk);
  for (const wk of ptWkNums) {
    if (!mergedWkMap.has(wk)) {
      mergedWkMap.set(wk, { milestone: `Practice Test (Week ${wk})`, total: '', rw: '', math: '' });
    }
  }
  // Sort by week number; fall back to original testRows if no week data was parsed
  const allTestRows = mergedWkMap.size > 0
    ? [...mergedWkMap.entries()].sort(([a], [b]) => a - b).map(([, r]) => r)
    : testRows;
...
function validateDerived(d, gamePlan) {
  ...
  if (gpDomains.length === 0) return; // nothing to validate
