...
function slide4Domains(d, n) {
  ...
  function domainRow(dom, isLast, highRank) {
    ...
    const rowBg       = isHigh ? '#FFF8F7' : isMed ? '#FFFBF4' : isProtect ? '#F0FBF6' : 'white';
    ...
    const rankBadge = (isHigh && highRank != null)
      ? `<div style="width:26px;height:26px;border-radius:50%;background:#B84A2E;color:white;font-size:13px;font-weight:800;display:flex;align-items:center;justify-content:center;font-family:'Inter',sans-serif;flex-shrink:0;">${highRank}</div>`
      : '';

    return `<div style="display:flex;align-items:stretch;${isLast ? '' : 'border-bottom:1px solid #E5E7EB;'}background:${rowBg};">
      <div style="width:7px;background:${borderColor};flex-shrink:0;border-radius:0;"></div>
      <div style="flex:1;display:flex;align-items:center;justify-content:space-between;padding:16px 24px;">
        <div style="flex:1;min-width:0;">
          ...
          <div style="font-size:13px;font-weight:500;color:#64748B;font-style:italic;font-family:'Inter',sans-serif;margin-left:${isHigh && highRank != null ? '36px' : '0'};">${esc(dom.performance)}</div>
        </div>
        <div style="display:flex;align-items:center;gap:14px;flex-shrink:0;margin-left:16px;">
          <span style="background:${badgeBg};color:${badgeText};font-size:11px;font-weight:800;padding:5px 12px;border-radius:6px;letter-spacing:0.07em;font-family:'Inter',sans-serif;white-space:nowrap;text-transform:uppercase;">${badgeLabel}</span>
          <div style="font-size:15px;font-weight:700;color:${goalColor};font-family:'Inter',sans-serif;white-space:nowrap;min-width:90px;text-align:right;">→ ${esc(dom.target)}</div>
        </div>
      </div>
    </div>`;
  }
  ...
  return `<section style="width:1920px;height:1080px;display:flex;flex-direction:column;position:relative;overflow:hidden;page-break-after:always;">
    ...
    <div style="flex:1;padding:48px 80px 80px;display:flex;flex-direction:column;background:#F8FAFC;">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;flex:1;margin-bottom:24px;">
        <!-- Left column -->
        <div style="border-radius:12px;overflow:hidden;border:1px solid #E5E7EB;display:flex;flex-direction:column;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
          <div style="background:${d.rwColor};padding:22px 28px;flex-shrink:0;">
            <div style="font-size:17px;font-weight:800;color:white;font-family:'Inter',sans-serif;">${esc(d.rwSectionLabel)}</div>
            <div style="font-size:14px;color:rgba(255,255,255,0.85);margin-top:4px;font-family:'Inter',sans-serif;">Current: ${rwCurDisplay}${d.isACT ? '' : ` | Target: ${rwTargetDisplay}`}</div>
          </div>
          <div style="flex:1;display:flex;flex-direction:column;min-height:0;">${makeRows(d.rwDomains)}</div>
        </div>
        <!-- Right column -->
        <div style="border-radius:12px;overflow:hidden;border:1px solid #E5E7EB;display:flex;flex-direction:column;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
          <div style="background:${d.mathColor};padding:22px 28px;flex-shrink:0;">
            <div style="font-size:17px;font-weight:800;color:white;font-family:'Inter',sans-serif;">${esc(d.mathSectionLabel)}</div>
            <div style="font-size:14px;color:rgba(255,255,255,0.85);margin-top:4px;font-family:'Inter',sans-serif;">Current: ${mathCurDisplay}${mathTargetDisplay ? ` | Target: ${mathTargetDisplay}` : ''}</div>
          </div>
...
async function proofreadData(d, gamePlan) {
  ...
  try {
    // Snapshot of what every slide currently shows (all editable text fields)
    const presentation = {
      // Slide 1 + 2: cover & standing
      thesis: d.thesis,

      // Slide 4: domain breakdown
      ...
      // Slide 8: phases roadmap  |  Slide 9: two gears (uses same shortName)
    ...
    // Condensed game plan for comparison (source of truth)
    const plan = {
      ...
      phases:         (gamePlan.phases || []).map(ph => ({
        ...
        hours:       ph.hours,
        homework:    ((ph.weeks || [])[0]?.homework || []).filter(h => h && h.trim()).slice(0, 3),
      })),
...
You have two documents: the authoritative GAME PLAN (source of truth) and the PRESENTATION (derived summary shown to the student and parent).
Your job — do ALL of the following:

1. GRAMMAR & PHRASING: Fix any grammar, punctuation, capitalization, or awkward phrasing in every text field of the presentation.
2. CONSISTENCY: Cross-check every presentation field against the game plan and fix any inconsistency — e.g. wrong domain priority labels, phase descriptions that contradict the plan, homework items that don't match the plan, a thesis that misrepresents the strategy.
3. HOMEWORK: Verify each phase's homework items match the game plan. Fix phrasing but keep the same tasks.
4. CONSTRAINTS: Do NOT change student names, score numbers, date ranges, week numbers, or hours numbers. Do NOT invent new content. Do NOT change the JSON structure or add/remove keys.

Return ONLY valid JSON with the exact same shape as PRESENTATION, with all corrections applied. No commentary.
...
${JSON.stringify(presentation, null, 2)}`;
    ...
    if (Array.isArray(corrected.phaseHomework)) {
      corrected.phaseHomework.forEach((ph, i) => {
        ...
        if (Array.isArray(ph.items) && ph.items.length > 0) d.phaseHomework[i].items = ph.items;
      });
    }
