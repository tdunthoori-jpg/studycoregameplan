...
async function proofreadData(d, gamePlan) {
  ...
  try {
    // Snapshot of what the presentation currently shows
    const presentation = {
      thesis: d.thesis,
    ...
    // Condensed game plan for comparison (only what's needed)
    const plan = {
      phases:         (gamePlan.phases || []).map(ph => ({ title: ph.title, description: ph.description, hours: ph.hours })),
      domainPriority: gamePlan.domainPriority || [],
      hook:           gamePlan.hook || gamePlan.thesis || '',
    };

    const prompt = `You are a quality-control editor for a professional SAT/ACT tutoring presentation. \
You have two documents: the authoritative GAME PLAN (source of truth) and the PRESENTATION (derived summary). \
Your job:
1. Fix any grammar, punctuation, capitalization, or awkward phrasing in presentation text fields.
2. Find inconsistencies between the game plan and presentation — e.g. wrong domain priorities, phase descriptions that contradict the plan, missing or wrong focus areas — and correct the presentation to match the game plan.
3. Do NOT invent new content. Do NOT change student names, score numbers, date ranges, or the overall structure.
4. Return ONLY valid JSON matching the exact shape of PRESENTATION below, with all corrections applied.
