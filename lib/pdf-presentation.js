...
function derive(gamePlan, studentData, studentName) {
  ...
  const parsedPhases = phases.map(ph => {
    const hMatch  = (ph.title || '').match(/(\d+)\s*(?:hours?|hrs?)(?!\s*\/\s*week)/i);
...
function slideHomework(d, n) {
  ...
  const itemFont  = count >= 5 ? 15 : 16;
  const nameFont  = count >= 5 ? 12 : 13;
  ...
  const phaseCards = d.phaseHomework.map((ph, i) => {
    const bullets = ph.items.map(item =>
      `<div style="display:flex;gap:12px;align-items:flex-start;margin-bottom:11px;">
        <div style="width:7px;height:7px;border-radius:50%;background:${ph.color};flex-shrink:0;margin-top:7px;"></div>
        <div style="font-size:${itemFont}px;font-weight:500;color:#334155;line-height:1.55;font-family:'Inter',sans-serif;">${esc(item)}</div>
      </div>`
    ).join('');
    return `<div style="background:white;border-radius:14px;padding:28px 32px;border-left:6px solid ${ph.color};box-shadow:0 2px 12px rgba(0,0,0,0.06);display:flex;flex-direction:column;">
      <div style="font-size:${nameFont}px;font-weight:700;color:${ph.color};letter-spacing:0.12em;text-transform:uppercase;margin-bottom:16px;font-family:'Inter',sans-serif;">PHASE ${ph.phaseIndex + 1} · ${esc(ph.shortName)}</div>
  ...
  return `<section style="width:1920px;height:1080px;display:flex;flex-direction:column;position:relative;overflow:hidden;page-break-after:always;">
    ...
    <div style="flex:1;padding:48px 80px 72px;display:flex;flex-direction:column;background:#F8FAFC;">
      ...
      <div style="display:grid;grid-template-columns:2fr 3fr;gap:40px;flex:1;min-height:0;align-items:stretch;">
        ...
        <div style="display:grid;grid-template-columns:${rightCols};grid-template-rows:${rightRows};gap:18px;">
          ${phaseCards}
        </div>
      </div>
    </div>
