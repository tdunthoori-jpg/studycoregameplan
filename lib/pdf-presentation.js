...
function slidePhase(d, ph, idx, n) {
  ...
  // Connector line sits at: label(14px) + margin-top(6px) + half-dot(4px) = 24px
  // For test ring: label(14px) + margin-top(3px) + half-ring(7px) = 24px  (same)
  const DOT_CENTER = 24;
  ...
  const nodeHtml = groups.map(grp => {
    ...
    if (grp.isTest) {
      ...
      return `<div style="flex:1;display:flex;flex-direction:column;align-items:flex-start;padding-right:8px;position:relative;">
        <div style="font-size:10px;font-weight:700;color:rgba(255,255,255,0.4);letter-spacing:0.5px;text-transform:uppercase;font-family:'Inter',sans-serif;line-height:14px;">${label}</div>
        <div style="width:14px;height:14px;border-radius:50%;border:1.5px solid #1A8870;display:flex;align-items:center;justify-content:center;margin:3px 0 8px;position:relative;z-index:1;">
          <div style="width:7px;height:7px;border-radius:50%;background:#1A8870;"></div>
        </div>
        <div style="font-size:11px;font-weight:600;color:#5ecaa5;line-height:1.4;font-family:'Inter',sans-serif;max-height:31px;overflow:hidden;">${txt}</div>
      </div>`;
    }
    ...
    const more       = grp.weeks.length > 1
      ? ` <span style="color:rgba(255,255,255,0.4);">+ ${grp.weeks.length - 1} more</span>`
      : '';
    return `<div style="flex:1;display:flex;flex-direction:column;align-items:flex-start;padding-right:8px;position:relative;">
      <div style="font-size:10px;font-weight:700;color:rgba(255,255,255,0.4);letter-spacing:0.5px;text-transform:uppercase;font-family:'Inter',sans-serif;line-height:14px;">${label}</div>
      <div style="width:8px;height:8px;border-radius:50%;background:${color};margin:6px 0 8px;position:relative;z-index:1;"></div>
      <div style="font-size:11px;color:rgba(255,255,255,0.75);line-height:1.4;font-family:'Inter',sans-serif;max-height:31px;overflow:hidden;">${esc(topic)}${more}</div>
    </div>`;
  }).join('');

  // Description: truncate to 120 chars
  const rawDesc   = (ph.shortDesc || ph.description || '').trim();
  const descShort = rawDesc.length > 120 ? rawDesc.slice(0, 119).replace(/\s+\S*$/, '') + '…' : rawDesc;
  ...
  return `<section style="width:1920px;height:1080px;display:flex;flex-direction:column;position:relative;overflow:hidden;page-break-after:always;">
    ...
    <div style="flex:1;display:flex;overflow:hidden;min-height:0;">
      ...
      <!-- Right: dark panel with description + timeline strip -->
      <div style="flex:1;padding:52px 72px 56px;background:#1E293B;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
        <!-- Phase description with color accent border -->
        <div style="font-size:17px;font-style:italic;color:rgba(255,255,255,0.7);line-height:1.6;font-family:'Inter',sans-serif;border-left:3px solid ${color};padding-left:18px;margin-bottom:20px;flex-shrink:0;">${esc(descShort)}</div>
        <!-- Horizontal rule -->
        <div style="height:1px;background:rgba(255,255,255,0.12);margin-bottom:0;flex-shrink:0;"></div>
        <!-- Timeline strip -->
        ${groups.length > 0 ? `
        <div style="position:relative;display:flex;align-items:flex-start;gap:0;width:100%;padding-top:16px;flex:1;min-height:0;">
          <!-- Connector line through dot centers -->
          <div style="position:absolute;top:${16 + DOT_CENTER}px;left:0;right:0;height:1px;background:rgba(255,255,255,0.12);z-index:0;"></div>
          ${nodeHtml}
        </div>` : ''}
      </div>
    </div>
