...
function slidePhase(d, ph, idx, n) {
  ...
  // Strip padding-top + node internals → connector position from strip container top
  // Node: label line-height(22px) + dot margin-top(16px) + half-dot(9px) = 47px from node top
  // Plus strip padding-top of 36px → connector at 83px from strip container top
  const STRIP_PAD   = 36;
  const DOT_FROM_NODE = 47; // label(22) + margin-top(16) + half-dot(9)
  const CONNECTOR_TOP = STRIP_PAD + DOT_FROM_NODE;
  ...
  const nodeHtml = groups.map(grp => {
    ...
    if (grp.isTest) {
      ...
      return `<div style="flex:1;display:flex;flex-direction:column;align-items:flex-start;padding-right:24px;position:relative;">
        <div style="font-size:16px;font-weight:700;color:rgba(255,255,255,0.38);letter-spacing:0.08em;text-transform:uppercase;font-family:'Inter',sans-serif;line-height:22px;">${label}</div>
        <div style="width:26px;height:26px;border-radius:50%;border:2px solid #1A8870;display:flex;align-items:center;justify-content:center;margin:12px 0 16px;position:relative;z-index:1;flex-shrink:0;">
          <div style="width:12px;height:12px;border-radius:50%;background:#1A8870;"></div>
        </div>
        <div style="font-size:18px;font-weight:600;color:#5ecaa5;line-height:1.6;font-family:'Inter',sans-serif;">${txt}</div>
      </div>`;
    }
    ...
    const more       = grp.weeks.length > 1
      ? `<div style="font-size:15px;color:rgba(255,255,255,0.32);margin-top:6px;font-family:'Inter',sans-serif;">+ ${grp.weeks.length - 1} more</div>`
      : '';
    return `<div style="flex:1;display:flex;flex-direction:column;align-items:flex-start;padding-right:24px;position:relative;">
      <div style="font-size:16px;font-weight:700;color:rgba(255,255,255,0.38);letter-spacing:0.08em;text-transform:uppercase;font-family:'Inter',sans-serif;line-height:22px;">${label}</div>
      <div style="width:18px;height:18px;border-radius:50%;background:${color};margin:16px 0 16px;position:relative;z-index:1;flex-shrink:0;"></div>
      <div style="font-size:18px;font-weight:500;color:rgba(255,255,255,0.85);line-height:1.6;font-family:'Inter',sans-serif;">${esc(topic)}${more}</div>
    </div>`;
  }).join('');
  ...
  return `<section style="width:1920px;height:1080px;display:flex;flex-direction:column;position:relative;overflow:hidden;page-break-after:always;">
    ...
    <div style="flex:1;display:flex;overflow:hidden;min-height:0;">
      ...
      <div style="flex:1;padding:56px 80px 56px;background:#1E293B;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
        ...
        <!-- Timeline section: flex:1 fills all remaining vertical space -->
        ${groups.length > 0 ? `
        <div style="flex:1;display:flex;flex-direction:column;min-height:0;">
          ...
          <div style="height:1px;background:rgba(255,255,255,0.15);flex-shrink:0;"></div>
          <!-- Strip: flex:1 fills remaining space; connector placed at calculated dot center -->
          <div style="flex:1;position:relative;display:flex;align-items:flex-start;padding-top:${STRIP_PAD}px;min-height:0;">
            <div style="position:absolute;top:${CONNECTOR_TOP}px;left:0;right:0;height:1px;background:rgba(255,255,255,0.15);z-index:0;"></div>
            ${nodeHtml}
          </div>
        </div>` : ''}
      </div>
