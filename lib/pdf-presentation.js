...
function derive(gamePlan, studentData, studentName) {
  ...
  if (isACT) {
    ...
    // Scan backwards for the last row that has both rw and math data — the final row is
    // often a "Target" summary row that may not have section scores populated.
    ...
    const _rowWithSections = [..._progRowsEarly].reverse().find(r => r.rw && r.math) || _progRowsEarly[_progRowsEarly.length - 1] || {};
    const _rwTarget        = parseTargetNum(_rowWithSections.rw)   || Math.round(target * 0.47);
    const _mathTarget      = parseTargetNum(_rowWithSections.math) || Math.round(target * 0.53);
  ...
  function psatRWPct(s) {
    const table = [
      ...
      [630,'85th'],[620,'83rd'],[610,'81st'],[600,'79th'],[590,'76th'],
  ...
  function psatMathPct(s) {
    const table = [
      ...
      [680,'95th'],[660,'94th'],[650,'94th'],[640,'93rd'],[630,'92nd'],
  ...
  function psatCompositePct(s) {
    const table = [
      ...
      [1350,'95th'],[1320,'94th'],[1300,'93rd'],[1280,'92nd'],[1260,'90th'],
  ...
  const baselinePct = isACT ? actPct(baseline)    : psatCompositePct(baseline);
  const rwPct       = isACT ? actPct(actEnglish)  : psatRWPct(rwCur);
  const mathPct     = isACT ? actPct(actMath)     : psatMathPct(mathCur);
  ...
  // Find the last row that has section-score data (rw/math) — the final row is often
  // a "Target" summary with no section breakdown, so scanning backwards is more reliable.
  const targetRow    = [...progRows].reverse().find(r => r.rw || r.math || r.total) || progRows[progRows.length - 1] || {};
  // For ACT: rwTargetStr = target composite string; mathTargetStr unused
  const rwTargetStr   = isACT
    ? (targetRow.total ? `${targetRow.total}+` : `${target}+`)
    : (targetRow.rw   || `${Math.round(target * 0.47)}+`);
  const mathTargetStr = isACT ? '' : (targetRow.math || `${Math.round(target * 0.53)}+`);
  ...
  // Build authoritative test list from programOverview.practiceTests
  // scoreProgression may only include a subset; programOverview has the full schedule
  const ptText    = gamePlan.programOverview?.practiceTests || '';
  const wkSection = ptText.match(/[Ww]eeks?\s+([\d][,\s\d–\-and]*)/);
  const ptWkNums  = wkSection
    ? [...wkSection[1].matchAll(/\d+/g)].map(m => parseInt(m[0])).filter(n => n >= 1 && n <= 60)
    : [];

  // Map scoreProgression rows by week so we keep their score data
  const scoreRowByWk = new Map();
  for (const r of testRows) {
    const wm = (r.milestone || '').match(/[Ww](?:eek)?\s*(\d+)/);
    if (wm) scoreRowByWk.set(parseInt(wm[1]), r);
  }
  // Canonical source: programOverview.practiceTests defines the authoritative test schedule.
  // Only show those weeks; pull score data from scoreProgression if available for that week.
  const allTestRows = ptWkNums.length > 0
    ? ptWkNums.map((wk, i) => {
        const row = scoreRowByWk.get(wk);
        return row || { milestone: `Practice Test ${i + 1} (Week ${wk})`, total: '', rw: '', math: '' };
      })
    : testRows;
  ...
  return {
    ...
    baselinePct, targetPct: isACT ? actPct(target) : psatCompositePct(target), rwPct, mathPct,
...
function slide4Domains(d, n) {
  ...
  function domainRow(dom, isLast, highRank) {
    ...
    const nameZone = `<div style="flex:1;display:flex;align-items:center;gap:12px;min-width:0;padding:0 24px;">
      ${rankBadge}
      <span style="font-size:19px;font-weight:700;color:#1B3A5C;font-family:'Inter',sans-serif;line-height:1.2;">${esc(dom.domain)}</span>
    </div>`;

    // Zone 2: current score — prominent, labeled, centered
    const currentZone = hasCurrent
      ? `<div style="width:160px;flex-shrink:0;display:flex;flex-direction:column;align-items:center;justify-content:center;border-left:1px solid #E5E7EB;border-right:1px solid #E5E7EB;padding:0 16px;">
          ...
          <div style="font-size:20px;font-weight:800;color:#1B3A5C;font-family:'Inter',sans-serif;white-space:nowrap;">${esc(dom.performance)}</div>
        </div>`
      : `<div style="width:160px;flex-shrink:0;"></div>`;

    // Zone 3: priority badge + goal
    const rightZone = `<div style="width:260px;flex-shrink:0;display:flex;align-items:center;justify-content:flex-end;gap:14px;padding:0 24px;">
      <span style="background:${badgeBg};color:${badgeText};font-size:12px;font-weight:800;padding:6px 14px;border-radius:6px;letter-spacing:0.07em;font-family:'Inter',sans-serif;white-space:nowrap;text-transform:uppercase;">${badgeLabel}</span>
      <div style="font-size:16px;font-weight:800;color:${goalColor};font-family:'Inter',sans-serif;white-space:nowrap;">→ ${esc(dom.target)}</div>
    </div>`;
...
function slidePhase(d, ph, idx, n) {
  ...
  // 9-10 weeks → 5 cols (2 rows). ≤8 → 4 cols. ≤6 → 3. ≤4 → fill one row.
  const gridCols = wkCount <= 4 ? (wkCount || 1) : wkCount <= 6 ? 3 : wkCount <= 8 ? 4 : 5;
  const cardHdr  = 10;

  // Distill sessionContent → a complete topic phrase by stripping boilerplate
  // and cutting at the first natural break (colon, paren, em-dash, or sentence end).
  function extractTopic(raw) {
    let s = raw.replace(/^\d[\d.]*-hour session\.\s*/i, '').trim();
    ...
    const topic = s.split(/[:(—–]|\.\s+/)[0].trim().replace(/[,;.]+$/, '');
    return topic ? (topic[0].toUpperCase() + topic.slice(1)) : raw.slice(0, 60);
  }

  // Card topic font scales with column count
  const topicFont = gridCols >= 5 ? 12 : gridCols === 4 ? 13 : 14;

  const weekCards = weeks.map((wk, wi) => {
    const wkNum      = wk.weekNum || wk.week || wk.weekNumber || String(wi + (ph.wkStart || 1));
    const sessionRaw = (wk.sessionContent || wk.sessions?.[0] || wk.focus || wk.focus_area || '').trim();
    const hwItems    = (wk.homework || []).filter(h => h && h.trim());
    const topic      = sessionRaw ? extractTopic(sessionRaw) : extractTopic(hwItems[0] || '');
    return `<div style="background:white;border-radius:10px;padding:18px 20px;border-top:3px solid ${color};box-shadow:0 2px 8px rgba(0,0,0,0.06);display:flex;flex-direction:column;overflow:hidden;min-height:0;">
      <div style="font-size:${cardHdr}px;font-weight:700;color:${color};letter-spacing:0.12em;text-transform:uppercase;margin-bottom:9px;flex-shrink:0;font-family:'Inter',sans-serif;">WEEK ${wkNum}</div>
      ${topic ? `<div style="font-size:${topicFont}px;font-weight:600;color:#1E293B;line-height:1.5;font-family:'Inter',sans-serif;">${esc(topic)}</div>` : ''}
    </div>`;
  }).join('');

  const hasWeeks = wkCount > 0;

  // Phase goal fallback (no weekly data): first 2 sentences of phaseComplete
  const rawGoal   = (ph.phaseComplete || '').replace(/^Phase\s+\d+\s+complete[.:]\s*/i, '').trim();
  const goalSents = rawGoal.split(/(?<=[.!?])\s+/).filter(s => s.trim().length > 5);
  const phaseGoal = goalSents.slice(0, 2).join(' ');

  // Description: 2 sentences max to leave room for the grid
  const fullDesc    = (ph.description || ph.shortDesc || '').trim();
  const descSents   = fullDesc.split(/(?<=[.!?])\s+/).filter(s => s.trim().length > 5);
  const description = descSents.slice(0, 2).join(' ');
  // Smaller font for phases with many weeks (less vertical space for description)
  const descFont    = wkCount >= 9 ? 17 : wkCount >= 5 ? 19 : 22;
  const divMargin   = wkCount >= 9 ? '18px 0 14px' : '26px 0 22px';
  ...
  return `<section style="width:1920px;height:1080px;display:flex;flex-direction:column;position:relative;overflow:hidden;page-break-after:always;">
    ...
    <div style="flex:1;display:flex;overflow:hidden;min-height:0;">
      ...
      <!-- Right: description + weekly breakdown -->
      <div style="flex:1;padding:48px 64px 52px;background:#F8FAFC;display:flex;flex-direction:column;overflow:hidden;min-width:0;">
        <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:14px;font-family:'Inter',sans-serif;flex-shrink:0;">WHAT WE'RE DOING</div>
        <div style="font-size:${descFont}px;font-weight:500;color:#334155;line-height:1.65;font-family:'Inter',sans-serif;flex-shrink:0;">${esc(description)}</div>
        <div style="height:1px;background:#E2E8F0;margin:${divMargin};flex-shrink:0;"></div>
        ${hasWeeks ? `
          <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:14px;flex-shrink:0;font-family:'Inter',sans-serif;">WEEK BY WEEK</div>
          <div style="display:grid;grid-template-columns:repeat(${gridCols},1fr);grid-auto-rows:minmax(120px,1fr);gap:14px;flex:1;min-height:0;overflow:hidden;">${weekCards}</div>
        ` : phaseGoal ? `
          <div style="font-size:11px;font-weight:700;color:#94A3B8;letter-spacing:0.18em;text-transform:uppercase;margin-bottom:14px;flex-shrink:0;font-family:'Inter',sans-serif;">PHASE GOAL</div>
          <div style="background:white;border-radius:16px;padding:40px 48px;border-left:6px solid ${color};box-shadow:0 2px 16px rgba(0,0,0,0.06);flex:1;display:flex;align-items:center;">
            <div style="font-size:22px;font-weight:700;color:#1B3A5C;line-height:1.65;font-family:'Inter',sans-serif;">${esc(phaseGoal)}</div>
          </div>
        ` : ''}
      </div>
    </div>
...
function validateDerived(d, gamePlan) {
  ...
  // Practice test count: allTestRows must reflect canonical programOverview.practiceTests count
  const gpPTText    = gamePlan.programOverview?.practiceTests || '';
  const gpWkSection = gpPTText.match(/[Ww]eeks?\s+([\d][,\s\d–\-and]*)/);
  const gpPTCount   = gpWkSection
    ? [...gpWkSection[1].matchAll(/\d+/g)].map(m => parseInt(m[0])).filter(n => n >= 1 && n <= 60).length
    : 0;
  if (gpPTCount > 0 && (d.allTestRows || []).length !== gpPTCount) {
    errors.push(
      `Practice test count: presentation=${(d.allTestRows || []).length} vs ` +
      `gamePlan.programOverview.practiceTests count=${gpPTCount}`
    );
  }
