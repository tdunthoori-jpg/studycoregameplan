...
function PriorityBadge({ priority }) {
  ...
  return <Text style={[s.badge, style]}>{priority || '—'}</Text>;
}
          ...
          <>
            ...
            {data.phases.map((phase, pi) => (
              <View key={pi}>
                ...
                {phase.weeks?.map((wk, wi) => (
                  <View key={wi} style={s.weekCard} wrap={false}>
                    ...
                    <View style={s.weekBody}>
                      ...
                      {wk.testTarget ? <Text style={s.weekTarget}>🎯 {wk.testTarget}</Text> : null}
                ...
                {phase.phaseComplete ? <Text style={s.phaseComplete}>✓ {phase.phaseComplete}</Text> : null}
              </View>
            ))}
          </>
        ) : null}
...
export async function buildGamePlanPdf(data, studentName) {
  const element = React.createElement(GamePlanDocument, { data, studentName: studentName || 'Student' });
