...
// ─── Canonical test-week filtering ─────────────────────────────────────────
// programOverview.practiceTests is the single source of truth for which weeks
// have practice tests. scoreProgression.rows may contain extra rows (e.g. from
// Claude writing more rows than canonical). Filter to canonical weeks only.

function parseCanonicalTestWeeks(practiceTestsText) {
  const wkSection = (practiceTestsText || '').match(/[Ww]eeks?\s+([\d][,\s\d–\-and]*)/);
  if (!wkSection) return [];
  return [...wkSection[1].matchAll(/\d+/g)]
    .map(m => parseInt(m[0]))
    .filter(n => n >= 1 && n <= 60);
}

function isTestRow(row) {
  const m = (row.milestone || '').toLowerCase();
  return (m.includes('test') || m.includes('practice') || m.includes('t#') || m.includes('wk')) &&
    !m.includes('baseline') && !m.includes('current') && !m.includes('target') &&
    !m.includes('goal') && !m.includes('actual');
}

function extractRowWeek(row) {
  const m = (row.milestone || '').match(/[Ww](?:eek)?\s*(\d+)/);
  return m ? parseInt(m[1]) : null;
}

function filterScoreProgressionRows(rows, canonicalWeeks) {
  if (canonicalWeeks.length === 0) return rows;
  const canonicalSet = new Set(canonicalWeeks);
  return rows.filter(row => {
    if (!isTestRow(row)) return true;   // always keep non-test rows (baseline, target, etc.)
    const wk = extractRowWeek(row);
    if (wk === null) return true;        // no week tag — keep to avoid dropping header rows
    return canonicalSet.has(wk);
  });
}

export async function buildGamePlanPdf(data, studentName) {
  const canonicalTestWeeks = parseCanonicalTestWeeks(data.programOverview?.practiceTests);
  const filteredData = {
    ...data,
    scoreProgression: data.scoreProgression
      ? {
          ...data.scoreProgression,
          rows: filterScoreProgressionRows(data.scoreProgression.rows || [], canonicalTestWeeks),
        }
      : data.scoreProgression,
  };
  const element = React.createElement(GamePlanDocument, { data: filteredData, studentName: studentName || 'Student' });
