            ...
            <View style={{ marginBottom: 6 }}>
              ...
              {data.scoreProgression.rows.map((row, i) => (
                <View key={i} style={i % 2 === 0 ? s.tableRow : s.tableRowAlt}>
                  ...
                  <Text style={[s.tableCell, { color: C.green }]}>{row.indicator}</Text>
                </View>
              ))}
            </View>
...
function validatePracticeTests(gamePlan) {
  const pts    = gamePlan.practiceTests || [];
  const errors = [];

  // 1. Ascending order and no duplicates
  for (let i = 1; i < pts.length; i++) {
    if (pts[i].week <= pts[i - 1].week) {
      errors.push(`practiceTests not in ascending week order: week ${pts[i].week} follows week ${pts[i - 1].week}`);
    }
  }
  const weekSet = new Set(pts.map(pt => pt.week));
  if (weekSet.size !== pts.length) errors.push('practiceTests contains duplicate week numbers');

  // 2. testTarget week cards that reference a practice test must point to a week in practiceTests.
  // testTarget is also used for general score goal lines (e.g. "Target: 1400+ | R/W: 700+") —
  // those do not reference a specific practice test and must not be flagged.
  // Baseline/diagnostic tests in early weeks use "Practice Test #1" language but are not
  // scheduled checkpoints — exclude them from the practiceTests consistency check.
  const isPracticeTestTarget = str =>
    /practice\s*test|^\s*pt\s*\d/i.test(str) && !/baseline|diagnostic/i.test(str);
  for (const phase of gamePlan.phases || []) {
    for (const wk of phase.weeks || []) {
      if (!wk.testTarget || !isPracticeTestTarget(wk.testTarget)) continue;
      const m = (wk.weekNum || '').match(/^(\d+)/);
      if (!m) continue;
      const cardWeek = parseInt(m[1]);
      if (!weekSet.has(cardWeek)) {
        errors.push(`Week ${wk.weekNum} has testTarget "${wk.testTarget}" but week ${cardWeek} is not in practiceTests`);
      }
    }
  }

  // 3. phaseComplete practice test count must match entries in that phase's week range
  for (const phase of gamePlan.phases || []) {
    if (!phase.phaseComplete) continue;
    const countMatch = phase.phaseComplete.match(/(\d+)\s+practice\s+test/i);
    if (!countMatch) continue;
    const statedCount = parseInt(countMatch[1]);

    // Parse phase week range from title (e.g. "Weeks 4–8" or "Week 4")
    const rangeMatch = (phase.title || '').match(/[Ww]eeks?\s+(\d+)(?:\s*[–\-]\s*(\d+))?/);
    if (!rangeMatch) continue;
    const phaseStart = parseInt(rangeMatch[1]);
    const phaseEnd   = rangeMatch[2] ? parseInt(rangeMatch[2]) : phaseStart;
    const actualCount = pts.filter(pt => pt.week >= phaseStart && pt.week <= phaseEnd).length;

    if (statedCount !== actualCount) {
      errors.push(
        `phaseComplete states ${statedCount} practice test(s) for weeks ${phaseStart}–${phaseEnd}, ` +
        `but practiceTests has ${actualCount} entries in that range`
      );
    }
  }
  ...
  if (errors.length > 0) {
    throw new Error(
      `[validatePracticeTests] Practice test consistency check failed:\n` +
      errors.map(e => `  • ${e}`).join('\n')
    );
  }
}
