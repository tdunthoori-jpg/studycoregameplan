 ...
 *
 * Session intensity is set by score gap:
 *   ≤100 pts → 1x/week, 1 hr
 *   ≤200 pts → 2x/week, 1 hr     ← anchor: 1300→1500 (+200) = 20 hrs, 10-week plan
 *   ≤300 pts → 2x/week, 1.5 hrs
 *    >300 pts → 2x/week, 1.5 hrs (capped)
 *
 * totalHours = sessionsPerWeek × sessionHours × planWeeks, clamped [10, 40].
 * This ensures the three numbers are always internally consistent.
 *
...
export function getRecommendation(gap, weeksAvailable) {
  ...
  // Session intensity from gap — determines pace, not total hours
  ...
  // totalHours = sessions × session length × weeks, clamped [10, 40]
  // This keeps all three numbers internally consistent.
  const sessionHrs = parseSessionHours(sessionLength);
  let totalHours = sessionsPerWeek * sessionHrs * planWeeks;
