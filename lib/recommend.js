 ...
 *
 * Hours formula: gap ÷ 10, clamped to [10, 40], rounded to nearest 5.
 * Calibrated anchor: 1300 → 1500 (+200 pts) = 20 hrs, 10-week plan.
 *
 * Plan length is snapped DOWN to the nearest standard tier based on
 * actual weeks until the test date:
 *   ≤ 9 weeks  → 8-week plan
 *   10–14 wks  → 10-week plan
 *   15–19 wks  → 15-week plan
 *   ≥ 20 wks   → 20-week plan
 *
 * Sessions/week and session length are derived from hours ÷ planWeeks.
 */
...
export function getRecommendation(gap, weeksAvailable) {
  ...
  // Hours: 1 hr per 10 points, clamped [10, 40], rounded to nearest 5
  let totalHours = Math.round((gap / 10) / 5) * 5;
  totalHours = Math.max(10, Math.min(40, totalHours));
  ...
  if (weeksAvailable && weeksAvailable > 0) {
    ...
    // No test date — pick a sensible default based on gap
    if (gap <= 100) planWeeks = 8;
    else if (gap <= 200) planWeeks = 10;
    else if (gap <= 300) planWeeks = 15;
    else planWeeks = 20; // default to 20 when no date; 40 reserved for far-out test dates
  }

  // Sessions/week and session length derived from hours-per-week rate
  const hrsPerWeek = totalHours / planWeeks;
  let sessionsPerWeek, sessionLength;
  if (hrsPerWeek < 1.5) {
    sessionsPerWeek = 1; sessionLength = '1 hr';
  } else if (hrsPerWeek < 2.5) {
    sessionsPerWeek = 2; sessionLength = '1 hr';
  } else if (hrsPerWeek < 3.5) {
    sessionsPerWeek = 2; sessionLength = '1.5 hrs';
  } else {
    sessionsPerWeek = 3; sessionLength = '1 hr';
  }
